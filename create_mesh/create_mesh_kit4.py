...
class electrodes_position():    
   ...
   def __init__(self, L, per_cover, rotate, anticlockwise=True):
      ...
      if not isinstance(per_cover, float): raise ValueError("per_cover must be a float.")
      ...
      if per_cover>1: raise ValueError("per_cover must be equal or less than 1. Example (75%): per_cover=0.75 ")
...
electrodes_obj = electrodes_position(L=L, per_cover=0.45, rotate=rotate, anticlockwise=False) 
...
for i in range(L):
   ...
   for theta in np.linspace(theta0, thetai, n_in):
      ...
      tag_list[counter] = gmsh.model.occ.addPoint(radius*np.cos(theta), radius*np.sin(theta), 0.0)
...
cp3 = gmsh.model.occ.addPoint(-cp_distance, -cp_distance, 0.0, meshSize=mesh_size_center)
...
for i in range(L):
   gmsh.model.addPhysicalGroup(1, tag_list[electrode_idx[i]], i+1, name="Elektrode"+str(i+1))
