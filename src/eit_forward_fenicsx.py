...
from dolfinx.fem import (Function, functionspace, assemble_scalar, form, Expression, assemble)
...
class EIT():
    def __init__(self, L, Inj, z, backend="PETSc", mesh_name="EIT_disk.msh"):
        ...
        assert Inj.shape[-1] == L, "Size of injection pattern should match the number of electrodes"
        ...
        self.omega, _, facet_markers = gmshio.read_from_msh(mesh_name, MPI.COMM_WORLD, gdim=2)
    ...
    def assemble_lhs(self):
        ...
        M.resize(self.dofs+self.L+1, self.dofs+self.L+1) # Extra row for zero average condition 
    ...
    def forward_solve(self, sigma, Inj=None):
        ...
        if self.backend == "PETSc":
            ...
            petsc_mat.setValues([self.dofs+self.L], [self.dofs+self.L], [[0.0]], PETSc.InsertMode.INSERT_VALUES)
