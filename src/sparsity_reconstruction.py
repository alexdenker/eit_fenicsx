...
from dolfinx.fem import (Constant, Function, form, dirichletbc, 
...
class L1Sparsity(Reconstructor):
    def __init__(self, eit_solver: EIT, 
        ...
        a = (ufl.inner(kappa * ufl.grad(self.eit_solver.u), ufl.grad(self.eit_solver.phi)) + self.eit_solver.u * self.eit_solver.phi) * ufl.dx
    ...
    def forward(self, Umeas, **kwargs):
        ...
        L = - ufl.inner(ufl.grad(u_placeholder), ufl.grad(p_placeholder)) * self.eit_solver.phi * ufl.dx
        ...
        with tqdm(total=self.max_iter, disable=disable) as pbar:
            for step in range(self.max_iter):
                ...
                sigma_j.x.array[:] = self.sigma_background.x.array[:] + sigma_iter.x.array[:]
                ...
                Dsigma_sum = self.compute_gradient(u_list, p_list, b, L, u_placeholder, p_placeholder)

                if step > 0:
                    step_size = self.compute_step_size(sigma_iter, Dsigma_sum, last_sigma, last_gradient, alpha, Umeas, loss_vals)
                else:
    ...
    def h1_norm(self, sigma):
        scalar_h1 = (ufl.inner(ufl.grad(sigma), ufl.grad(self.eit_solver.phi)) + sigma * self.eit_solver.phi) * ufl.dx
        b_scalar_h1 = assemble_vector(form(scalar_h1))
        return np.sum(np.abs(b_scalar_h1.array[:]))

    def compute_step_size(self, sigma_iter, Dsigma_sum, last_sigma, last_gradient, alpha, Umeas, loss_vals):
        ...
        while True: 
            ...
            sigma_test.x.array[:] = self.sigma_background.x.array[:] + sigma_guess.x.array[:]
            ...
            if all([new_loss + step_size*self.s*second_term > l for l in loss_vals]):
    ...
    def visualise(self):
        ...
        cells = self.eit_solver.omega.geometry.dofmap.reshape((-1, self.eit_solver.omega.topology.dim + 1))
