...
def get_fcunet_model(in_dim, in_res=64, in_ch=1, out_ch=1, scales=5, skip=4,
                   channels=(32, 32, 64, 64, 128, 128), use_sigmoid=False,
                   use_norm=True):
    assert (1 <= scales <= 6)
    skip_channels = [skip] * (scales)
    return FCUNetModel(in_dim=in_dim, in_res=in_res, in_ch=in_ch, out_ch=out_ch, channels=channels[:scales],
                skip_channels=skip_channels, use_sigmoid=use_sigmoid,
                use_norm=use_norm)

class FCUNetModel(nn.Module):

    def __init__(self, in_dim, in_ch, out_ch, channels, skip_channels, in_res = 64,
                 use_sigmoid=False, use_norm=True):
        super(FCUNetModel, self).__init__()
        assert (len(channels) == len(skip_channels))
        self.scales = len(channels)
        self.use_sigmoid = use_sigmoid

        self.in_res = in_res
        self.initial_linear = nn.Linear(in_dim, self.in_res*self.in_res)
        self.initial_linear.weight.data.fill_(0.0)
        self.initial_linear.bias.data.fill_(0.0)
    ...
    def linear_layer(self, x):
        ...
        x_img = self.initial_linear(x).reshape(x.shape[0], 1, self.in_res, self.in_res)
        return nn.functional.interpolate(x_img, (256, 256), mode = "bilinear")

    def forward(self, x0):
        x0 = self.linear_layer(x0)
        xs = [self.inc(x0), ]
    ...
    def __init__(self, in_ch, out_ch, kernel_size=3, use_norm=True):
        ...
        if use_norm:
            self.conv = nn.Sequential(
                ...
                nn.GroupNorm(num_channels=out_ch, num_groups=16),
