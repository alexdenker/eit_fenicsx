...
class FCUNetModel(nn.Module):
    def __init__(
        ...
        for i in range(1, self.scales):
            self.down.append(
                DownBlock(in_ch=channels[i - 1], out_ch=channels[i], use_norm=use_norm)
            )
        for i in range(1, self.scales):
            self.up.append(
                UpBlock(
...
class UNet(nn.Module):
    def __init__(
        self, in_ch, out_ch, channels, skip_channels, use_sigmoid=False, use_norm=True
    ):
    ...
    def __init__(self, in_ch, out_ch, skip_ch=4, kernel_size=3, use_norm=True):
        ...
        if use_norm:
            self.conv = nn.Sequential(
                ...
                nn.Conv2d(
                    in_ch + skip_ch, out_ch, kernel_size, stride=1, padding=to_pad
                ),
                nn.GroupNorm(num_channels=out_ch, num_groups=16),
                nn.LeakyReLU(0.2, inplace=True),
                nn.Conv2d(out_ch, out_ch, kernel_size, stride=1, padding=to_pad),
