...
def sample_inclusions(numInc: int, test_step: int = 200, tolerance: int = 30):
    h, k, a, b, alpha = np.zeros(numInc), np.zeros(numInc), np.zeros(numInc), np.zeros(numInc), np.zeros(numInc)
...
def gen_conductivity(x1: NDArray[np.float64], x2: NDArray[np.float64], max_numInc: int, texture: bool = False, backCond: float = 1.0) -> NDArray[np.float64]:
    ...
    if isinstance(texture, bool):
        if texture:
            ...
            for i in range(numInc):
                cond_opt = [np.random.rand() * 0.29 + 0.01, np.random.rand() * 1.0 + 2.0]
                cond_idx = np.random.randint(2)
                cond[i] = cond_opt[cond_idx]
