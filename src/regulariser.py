...
def create_smoothness_regulariser(omega, save_name: str, corrlength: float = 1.0, std: float = 0.3):
    """
...
def plot_samples_from_prior(L, triangulation):
    ...
    for i in range(4):
        ...
        im = axes[i].tripcolor(triangulation, samples[:,i].flatten(), cmap='jet', shading='flat')
        axes[i].axis('image')
