...
class CEMModule(nn.Module):
    def __init__(self, eit_solver, kappa=5e-3, gradient_smooting=False):
        ...
        self.DGFunctionSpace = FunctionSpace(self.eit_solver.omega, ("DG", 0))
    ...
    def forward(self, sigma):
        ...
        return self.cem_function.apply(sigma, self.eit_solver, self.DGFunctionSpace, self.gradient_solver, self.b, self.bilinear_form, self.bc,self.gradient_smooting)
...
class CEMTorch(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        grad_torch = torch.zeros(ctx.sigma_torch.shape, dtype=torch.double)
