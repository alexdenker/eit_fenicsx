...
class EllipsesDataset(Dataset):
    ...
    def __init__(self, part="train", base_path="dataset", inj_mode="all_against_1"):
        assert part in ["train", "val", "test"], "Part has to be either train, val or test"
        assert inj_mode in ["all_against_1", "all"], "inj_mode has to be either all_against_1 or all"
        ...
        if self.part == "test":
            # load noisy data 
            self.Umeas_files = [f for f in os.listdir(os.path.join(self.base_path, self.part)) if f.startswith("Umeas_noisy")]
            self.Umeas_files.sort(key = lambda x: int(x.split(".")[0].split("_")[-1]))
        else:
            # load clean data
            self.Umeas_files = [f for f in os.listdir(os.path.join(self.base_path, self.part)) if f.startswith("Umeas")]
            self.Umeas_files.sort(key = lambda x: int(x.split(".")[0].split("_")[-1]))
    ...
    def visualise_phantom(self, sigma):
        ...
        im = ax.tripcolor(self.tri, sigma.cpu().numpy().flatten(), cmap='jet', shading='flat', vmin=0.01, vmax=4.0,edgecolors='k')
        ax.axis('image')
    ...
    def interpolate_to_image(self, sigma, fill_value=0.0, res=256):
        ...
        pos = [[(coordinates[cells[i,0],0] + coordinates[cells[i,1],0] + coordinates[cells[i,2],0])/3.
                    ,(coordinates[cells[i,0],1] + coordinates[cells[i,1],1] + coordinates[cells[i,2],1])/3.] for i in range(cells.shape[0])]
        pos = np.array(pos)
        ...
        for i in range(sigma.shape[0]):
            ...
            interp = LinearNDInterpolator(pos, torch.clone(sigma[i,:]).cpu().numpy(), fill_value=fill_value)
            sigma_grid = interp(pixcenters)
                
            sigma_pix = np.flipud(sigma_grid.reshape(res, res))
            sigma_pix = torch.from_numpy(sigma_pix.copy()).float().to(sigma.device).unsqueeze(0)
...
if __name__ == "__main__":
    ...
    ax.imshow(s_pix[0,0,:,:].cpu().numpy(), cmap='jet', vmin=0.01, vmax=4.0,)
    plt.show()